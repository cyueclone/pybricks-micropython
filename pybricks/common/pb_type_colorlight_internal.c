...
STATIC mp_obj_t common_ColorLight_internal_blink(size_t n_args, const mp_obj_t *pos_args, mp_map_t *kw_args) {
    ...
    size_t cells_size = sizeof(pbio_color_t) * (durations_len + 1);
    #if MICROPY_MALLOC_USES_ALLOCATED_SIZE
    ...
    for (int i = 0; i < durations_len; i++) {
        const pbio_color_hsv_t *hsv = i % 2 ? pb_type_Color_get_hsv(mp_const_none) : pb_type_Color_get_hsv(color_in);
        cells[i] = PBIO_COLOR_ENCODE(hsv->h, hsv->s, hsv->v, pb_obj_get_positive_int(mp_iternext(durations_iter)));
    }

    // sentinel value
    cells[durations_len] = PBIO_COLOR_END;
...
STATIC mp_obj_t common_ColorLight_internal_animate(size_t n_args, const mp_obj_t *pos_args, mp_map_t *kw_args) {
    ...
    for (int i = 0; i < colors_len; i++) {
        const pbio_color_hsv_t *hsv = pb_type_Color_get_hsv(mp_iternext(colors_iter));
        cells[i] = PBIO_COLOR_ENCODE(hsv->h, hsv->s, hsv->v, interval);
    }

    // sentinel value
    cells[colors_len] = PBIO_COLOR_END;
